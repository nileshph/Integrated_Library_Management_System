...
if ($_SERVER ["REQUEST_METHOD"] == "GET") {
	...
	$sql = "select * from item where title like '%".$keyword."%' and active_ind = 1";
	$resultset = $conn->query ( $sql );

	$sql = "select * from item where publication like '%".$keyword."% and active_ind = 1'";
	$resultset1 = $conn->query ( $sql );

	$sql = "select * from item where isbn = '".$keyword."' and active_ind = 1";
